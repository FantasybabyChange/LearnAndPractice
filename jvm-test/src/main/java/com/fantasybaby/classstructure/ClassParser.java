...
public class ClassParser {
	...
	private String getMethodSignature(int accessFlag, String methodName, String descriptor) {
		...
		if("<init>".equals(methodName)){//如果是构造函数
	...
	private String getParamStrings(String params) {
		...
		while (pos < length) {//如果还未到参数字符串最后
			...
			if (ch == '[') {//如果数组标志
				...
				case 'L':
					...
					int begin = pos;
					for (; pos < length; pos++) {
						if (params.charAt(pos) == ';') {//找到类全名结束位置
		...
		if (paramList.length() > 0) {//如果有参数，删除最后一个,
